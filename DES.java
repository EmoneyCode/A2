...
class DES extends Feistel
{
    ...
    static int[][] ks = {
        ...
        {58,34,17,43,3,36,52,11,50,57,2,25,51,18,9,44,27,41,42,49,19,10,1,60,7,
         45,20,39,22,21,28,15,53,38,4,14,46,6,23,13,63,37,30,62,61,47,5,12},
...
class DESround  extends FeistelFunction
{
    ...
    int[] runThroughSboxes(int[] data)
    {
        ...
        for(int i = 1; i < 9; i++){
            ...
            //bits relating to number at row,col
            String resultBits = Integer.toBinaryString(S[i][row][col]);

            //adds necessary leading zeros to bit string
            String leadingZeros = "";
            for(int j = 0; j < 4 - resultBits.length(); j++){

            leadingZeros += '0';

            }
            resultBits = leadingZeros + resultBits;

            //adds bits to array
            int count = 0;
            for(int j = (i-1)*4; j < (i-1)*4 + 4; j++){
                result[j] = resultBits.charAt(count) - '0';
                count++;
            }
        }
    ...
    */
    ...
    {
        //expands data
        int[] expandedData = Utils.applyPermut(expandP, data);

        expandedData = Utils.XOR(expandedData, subkey);

        int[] newData = runThroughSboxes(expandedData);

        newData = Utils.applyPermut(P, newData);

        return newData; // here to please the compiler
   }// round method
